...
import { ArrowLeft, Eye, EyeOff, Lock, Mail, ShieldCheck, Store, User, Wrench } from "lucide-react";
...
const ROLE_OPTIONS: { key: UserRole; label: string; desc: string; icon: typeof User; color: string; bg: string }[] = [
  { key: "client", label: "Client", desc: "Book services & shop", icon: User, color: "text-emerald-600", bg: "bg-emerald-50 border-emerald-200" },
  { key: "provider", label: "Provider", desc: "Accept jobs & earn", icon: Wrench, color: "text-blue-600", bg: "bg-blue-50 border-blue-200" },
  { key: "admin", label: "Admin", desc: "Manage platform", icon: ShieldCheck, color: "text-red-600", bg: "bg-red-50 border-red-200" },
];
...
export default function LoginPage() {
  ...
  const emailError = email.trim() && !EMAIL_REGEX.test(email) ? "Enter a valid email address" : "";
  ...
  }, [isLoggedIn, isLoading, user, router, redirectTo]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success && result.requires2FA) {
      setRequires2FA(true);
      setUserIdFor2FA(result.data?.userId);
      setEmailFor2FA(result.data?.email);
      return;
    }
  ...
  const handle2FASubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Validate the TOTP with backend
      const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:3001";
      const res = await fetch(`${backendUrl}/api/auth/2fa/validate`, {
        method: "POST",
      ...
      // 2. We now have a valid JWT. Tell NextAuth to forge a session using otpMode bypass.
      const signInRes = await signIn("credentials", {
        ...
        userType: payload.data.user.userType,
  ...
  return (
    <main className="min-h-screen bg-gray-50 flex">
      ...
      <div className="hidden lg:flex flex-1 bg-(--color-primary) items-center justify-center p-12 relative overflow-hidden">
        <button className="absolute top-10 left-20 text-white flex items-center gap-2 cursor-pointer" onClick={() => router.push("/")}>
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12 mb-4">
        <div className="w-full max-w-md">
          <div className="lg:hidden flex items-center justify-between w-full mb-6">
            <button className="text-(--color-primary) flex items-center gap-2 cursor-pointer" onClick={() => router.push("/")}>
              <ArrowLeft /> Back
            </button>
          </div>
          ...
          <div className="w-full flex flex-col items-center lg:items-start">
            ...
            {(session as { error?: string; user?: { email?: string } })?.error === "EmailNotVerified" && (
              <div className="mb-4 w-full bg-amber-50 border border-amber-200 rounded-xl p-3 text-sm text-amber-700">
                Your email isn&apos;t verified yet.{" "}
                <button type="button" onClick={() => router.push(`/auth/verify-otp?email=${encodeURIComponent((session as { user?: { email?: string } })?.user?.email || "")}`)}
                  className="font-semibold text-(--color-primary) hover:underline">Verify now</button>
              </div>
            )}

            {requires2FA ? (
              <form onSubmit={handle2FASubmit} className="space-y-5 w-full mt-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 animate-fadeIn">
                <div className="text-center mb-4">
                  <div className="w-12 h-12 bg-(--color-primary-light) text-(--color-primary) rounded-full flex items-center justify-center mx-auto mb-3">
                    <ShieldCheck size={24} />
                  </div>
                ...
                <div>
                  <input 
                    ...
                    placeholder="000000"
                    className="w-full text-center tracking-[0.5em] text-2xl py-3 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-(--color-primary) focus:border-transparent font-mono" 
                  />
                </div>
                <button type="submit" disabled={loading || twoFactorToken.length !== 6}
                  className="w-full py-3.5 bg-(--color-primary) text-white rounded-full font-semibold hover:opacity-90 transition-colors disabled:opacity-50 flex items-center justify-center">
                  {loading ? "Verifying..." : "Verify Code"}
                </button>
                <button type="button" onClick={() => setRequires2FA(false)} className="w-full py-2 text-sm text-gray-500 hover:text-gray-700 font-medium">
                  Cancel
                </button>
              </form>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5 w-full">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <Link href="/forgot-password" className="text-sm text-(--color-primary) font-medium hover:underline">Forgot password?</Link>
                  </div>
                  <div className="relative">
                ...
                <p className="text-gray-500 mb-4 text-sm lg:text-md">
                  ...
                  <Link href="/signup" className="text-(--color-primary) font-medium hover:underline">Sign up</Link>
                </p>
                ...
                <button type="button" onClick={() => signIn("facebook", { callbackUrl: "/" })}
                  className="w-full py-3.5 mt-4 border border-gray-200 bg-white text-gray-900 text-sm lg:text-md rounded-full font-semibold hover:bg-gray-50 transition-colors flex items-center justify-center gap-3">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="#166FE5">
