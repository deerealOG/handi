...
import { Check, Eye, EyeOff, Loader2, Mail, Lock, Phone, User as UserIcon, Building2, Store, MapPin } from "lucide-react";
...
export default function VendorSignup() {
  ...
  const [formData, setFormData] = useState({
    ...
    phone: "",
    password: "",
    userType: "provider" as const, // Vendor maps to provider+business in backend
    providerSubType: "business" as const,
    businessName: "",
    bio: "",
    city: "",
    state: "",
  ...
  const nextStep = () => {
    ...
    if (step === 1) {
      setFieldTouched({
        firstName: true, lastName: true, email: true, phone: true, businessName: true, password: true,
      });
      if (!canProceedStep1) {
    ...
    <div className="w-full">
      {step === 1 && (
        <div className="space-y-4 animate-fadeIn">
          <div>
            <h3 className="text-lg font-semibold text-gray-900 mb-1">Store Owner Info</h3>
            <p className="text-sm text-gray-500">Create your vendor account</p>
          </div>
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input type="text" value={formData.businessName}
            ...
            className="w-full pl-10 pr-3 py-2.5 bg-white border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-(--color-primary)" placeholder="My Shop" />
        </div>
        {validateField("businessName", formData.businessName) && <p className="mt-1 text-xs text-red-500">{validateField("businessName", formData.businessName)}</p>}
      </div>
      ...
      <div>
        ...
        {validateField("password", formData.password) ? (
          ...
          <button onClick={nextStep} disabled={!agreed}
            className="w-full bg-(--color-primary) text-white py-3.5 rounded-full font-semibold hover:opacity-90 transition-opacity disabled:opacity-50 flex justify-center items-center mt-6">
            Continue
          </button>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="space-y-4 animate-fadeIn">
          <div>
            ...
            <p className="text-sm text-gray-500">Add details so clients can find your store</p>
          </div>
