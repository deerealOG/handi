...
export default function CheckoutPage() {
  ...
  const canProceedPayment = true; // All methods proceed via Paystack or wallet

  const handlePlaceOrder = async () => {
    if (paymentMethod === "wallet") {
      // Wallet payment — simulate
      setProcessing(true);
    ...
    // Card or Bank Transfer — invoke Paystack popup
    setProcessing(true);
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
    const handler = (window as any).PaystackPop.setup({
      key: paystackKey,
      email: user?.email || "guest@handi.ng",
      amount: total * 100, // Paystack expects kobo
      currency: "NGN",
      ref: orderId,
      channels: paymentMethod === "bank" ? ["bank", "bank_transfer"] : ["card"],
      metadata: {
        custom_fields: [
          {
          ...
          <div className="flex-1">
            ...
            {step === "payment" && (
              <div className="bg-white rounded-2xl shadow-sm p-6">
                ...
                {/* Card Details — handled by Paystack popup */}
                {paymentMethod === "card" && (
                  <div className="p-4 bg-gray-50 rounded-xl text-sm text-gray-600">
                    ...
                    <p>
                      Your card details will be securely collected by Paystack
                      when you place your order. We never store your card
                      information.
                    </p>
                    <div className="flex items-center gap-3 mt-3">
                      <span className="text-[10px] bg-white border border-gray-200 px-2 py-1 rounded font-semibold text-gray-500">
                        VISA
                      </span>
                      <span className="text-[10px] bg-white border border-gray-200 px-2 py-1 rounded font-semibold text-gray-500">
                        MASTERCARD
