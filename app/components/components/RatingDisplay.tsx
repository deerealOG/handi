    ...
    <View style={[styles.container, { gap: sizeConfig.gap }, style]}>
      ...
      {showCount && reviewCount > 0 && (
        <Text
          style={[
            styles.countText,
            { fontSize: sizeConfig.text - 2, color: colors.muted },
          ]}
        >
          ({reviewCount})
        </Text>
  ...
  return (
    <View style={[styles.starRow, { gap }]}>
      {[1, 2, 3, 4, 5].map((star) => {
        if (star <= fullStars) {
          return (
            <Ionicons key={star} name="star" size={size} color="#FACC15" />
          );
        }
        if (star === fullStars + 1 && hasHalfStar) {
          return (
            <Ionicons key={star} name="star-half" size={size} color="#FACC15" />
          );
        }
        return (
          <Ionicons
    ...
    >
      ...
      {reviewCount !== undefined && (
        <Text
          style={[
            styles.badgeCount,
            { fontSize: sizeConfig.text - 1, color: colors.muted },
          ]}
        >
          ({reviewCount})
        </Text>
  ...
  return (
    ...
    >
      <View style={styles.heroMain}>
        <Text style={[styles.heroRating, { color: colors.text }]}>
          {rating.toFixed(1)}
        </Text>
      ...
      <View style={styles.heroReviews}>
        <Text style={[styles.heroReviewCount, { color: colors.text }]}>
          {reviewCount.toLocaleString()}
        </Text>
        <Text style={[styles.heroReviewLabel, { color: colors.muted }]}>
          Reviews
        </Text>
      </View>
    </View>
        ...
        return (
          <View key={item.stars} style={styles.breakdownRow}>
            <View style={styles.breakdownLabel}>
              <Text style={[styles.breakdownStars, { color: colors.text }]}>
                {item.stars}
              </Text>
