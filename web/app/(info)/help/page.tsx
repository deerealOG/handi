        ...
        <div className="max-w-4xl mx-auto">
          <div className="grid sm:grid-cols-2 gap-6">
            {HELP_CATEGORIES.map((category) => (
              ...
              >
                <div className="w-12 h-12 rounded-xl bg-green-50 flex items-center justify-center mb-4 group-hover:bg-(--color-primary) transition-colors">
                  <category.icon
                    size={24}
      ...
      <section className="py-12 px-4 sm:px-6 lg:px-8 bg-gray-50">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {[
              { label: "How to book a service", href: "/faq#booking" },
              { label: "Payment methods", href: "/faq#payments" },
              { label: "Cancellation policy", href: "/faq#cancellation" },
              { label: "Become a provider", href: "/become-provider" },
              { label: "Account settings", href: "/settings" },
              { label: "Refund requests", href: "/faq#refunds" },
            ].map((topic) => (
              <Link
                key={topic.label}
                href={topic.href}
                className="bg-white px-4 py-2 rounded-[50px] text-sm hover:bg-primary hover:text-white transition-colors"
              >
                {topic.label}
              </Link>
            ))}
